 ...
 * @since 1.0.2
 */
class CCTS_REST_Ced_Api_Controller {
	...
	public function ced_update_product_quantity( $request ) {
		...
		if ( empty( $params['skus'] ) ) {
			return new WP_Error( 'ced_rest_missing_required_param', __( 'Required param missing.', 'cedcommerce-connector-for-tiktok-shop' ), array( 'status' => 406 ) );
		}
		$skus = $params['skus'];
		if ( ! rest_is_object( $skus ) ) {
			return new WP_Error( 'ced_rest_invalid_data_format', __( 'Invalid data format.', 'cedcommerce-connector-for-tiktok-shop' ), array( 'status' => 406 ) );
		}
		$successSkus = array();
		$errSkus     = array();
		foreach ( $skus as $sku ) {
			...
			$successSkus[ $sku ] = wc_update_product_stock( wc_get_product( $productId ), 1, 'decrease' );
		}
		return rest_ensure_response(
			array(
				'success'    => true,
				'successkus' => $successSkus,
				'errorskus'  => $errSkus,
			)
		);
	}
	...
	public function register_routes() {
		...
		register_rest_route(
			...
			array(
				'methods'             => 'GET',
				'callback'            => array( $this, 'ced_tiktok_get_order_status' ),
				'permission_callback' => array( $this, 'get_post_meta_permission' ),
			)
		);
	}
	...
	public function update_product_permission_cb() {
		if ( ! wc_rest_check_post_permissions( 'product', 'batch' ) ) {
			return new WP_Error( 'ced_rest_cannot_edit', __( 'Sorry, you are not allowed to update resources.', 'cedcommerce-connector-for-tiktok-shop' ), array( 'status' => rest_authorization_required_code() ) );
		}
	...
	public function get_post_meta_permission() {
		if ( ! wc_rest_check_post_permissions( 'post', 'read' ) ) {
			return new WP_Error( 'ced_rest_cannot_edit', __( 'Sorry, you are not allowed to access resources.', 'cedcommerce-connector-for-tiktok-shop' ), array( 'status' => rest_authorization_required_code() ) );
		}
	 ...
	 * @return array
	 */
	public function ced_tiktok_get_order_status() {
	...
	public function ced_update_source_shipping_carrier( $request ) {
		...
		$check_updated = update_option( 'CCTS' . $marketplace . '_shipping_carriers', $data );
		if ( $check_updated ) {
			return true;
		}
		return false;
